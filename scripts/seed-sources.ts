...
## 용어 설명 (glossary)
목표는 "어려운 단어"를 뽑는 게 아니라, 이 기사를 이해하는 데 **금융·경제 전문지식이 필요한
용어**만 뽑는 것입니다. 기사에 실제로 등장한 용어 중 다음 분야에 해당하는 것만 최대 5개까지
glossary에 넣으세요 — 저축은행 업계 용어로 국한하지 마세요:
- 금융시장 이해에 필요한 용어 (예: 기준금리, 유동성, 순부채, FCF)
- 경제정책 이해에 필요한 용어 (예: 통화정책, 재정정책)
- 금융회사 업무 이해에 필요한 용어 (예: 여신, 수신, 지급여력)
- 투자·증권 관련 전문용어 (예: Capex, 배당수익률, 주주환원)
- 은행·저축은행 관련 전문용어 (예: BIS비율, 예대마진, 고정이하여신비율)
- 거시경제 관련 전문용어 (예: 하이퍼스케일러처럼 산업 구조를 나타내는 전문어 포함)
- 금융상품/금융규제 관련 전문용어 (예: PF, DSR, LTV)

다음처럼 그냥 어렵거나 생소할 뿐인 일반 단어는 절대 넣지 마세요 — "전망", "증가", "감소",
"투자", "시장", "기업", "성장", "전략", "은행", "대출", "이자", "세금"처럼 배경지식이 없어도
문맥으로 뜻을 알 수 있는 단어는 전문용어가 아닙니다.

설명(definition)은 금융경제 초보자를 대상으로, 사전적 정의를 그대로 옮기지 말고 쉽게 풀어서
한두 문장으로 쓰세요. 예: "FCF" → "기업이 영업으로 벌어들인 현금에서 설비투자 등에 필요한 돈을
빼고 실제로 자유롭게 쓸 수 있는 현금입니다."

context(선택)에는 이 용어가 "이 기사에서" 어떤 맥락으로 쓰였는지 한 문장으로 적으세요. 일반적인
용어 정의와 구분되는, 기사 고유의 문맥 정보만 담고 정의를 반복하지 마세요. 채울 내용이 마땅치
않으면 생략해도 됩니다.

핵심 용어 위주로 적게 뽑는 것이 많이 뽑는 것보다 낫습니다. 위 기준에 맞는 전문용어가 하나도
없는 기사는 드뭅니다 — 정말 없을 때만 빈 배열로 두세요.

## 중요도 기준 (importance) — 저축은행과 무관하게, 금융 시장 전체 관점에서 이 뉴스 자체의 비중
5 = 시장 전체에 즉각적 파급력 (예: 기준금리 결정, 대형 M&A, 금융위기 신호, 정부 부동산 대책 전면 개편)
4 = 금융권 전반이 주목할 주요 뉴스 (예: 대기업 실적 서프라이즈, 대형 은행 정책 변화)
3 = 업계 관계자에게 의미 있으나 대중 관심은 제한적 (예: 특정 업권 통계 발표)
2 = 참고 수준의 소규모 뉴스 (예: 단발성 인사, 소규모 계약)
1 = 뉴스 가치가 낮은 정형화된 정보 (예: 정기 공시, 반복 일정)

## 영향도 기준 (sb_impact_score) — 저축은행 업계 관점에서 이 뉴스가 미치는 영향
...
importance와 sb_impact_score는 서로 다른 관점입니다 — 저축은행 업계엔 중요해도(sb_impact_score
높음) 전체 시장 뉴스로는 소규모일 수 있고(importance 낮음), 반대의 경우도 있습니다. 두 값을
독립적으로 판단하세요.
